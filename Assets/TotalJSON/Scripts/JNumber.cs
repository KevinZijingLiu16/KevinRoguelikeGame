    ...
    public class JNumber : JValue
    {
        ...
        private static string numberFormatCheck(string str)
        {
            ...
            for (int index = 0; index < count; index++)
            {
                ...
                { // Number started with (minus) zero, only decimal point or E can follow
                    if (chr == '.')
                    {
                        state = 5;
                    }
                ...
                { // Digits before decimal point or E
                    if (chr == '.')
                    {
                        state = 5;
                    }
                ...
                { // Following digits after decimal point
                    if (chr == 'e' || chr == 'E')
                    {
                        state = 7;
                    }
                ...
                { // Need at least one digit after E plus/minus,
                    if (chr >= '0' && chr <= '9')
                    {
                        state = 9;
                    }
        ...
        private string safeFloatAsString(float value)
        {
            ...
            try
            {
                ...
                { // May not be true because rounding
                    return basicStr;
                }
            }
            catch (OverflowException)
